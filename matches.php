...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        $winnerTeamId = nullable_post_int('winner_team_id');
        ...
        if ($winnerTeamId !== null && !in_array($winnerTeamId, [$team1Id, $team2Id], true)) {
            $errors[] = 'Winner must be one of the participating teams.';
        }
        ...
        if (!$errors) {
            try {
                if ($matchId > 0) {
                    ...
                    //set_flash('success', 'Match updated and points recalculated.');
                } else {
                    $statement = $pdo->prepare(
                        'INSERT INTO matches (team1_id, team2_id, match_date, winner_team_id)
                         VALUES (:team1_id, :team2_id, :match_date, :winner_team_id)'
                    );
                    $statement->execute([
                        'team1_id' => $team1Id,
                        'team2_id' => $team2Id,
                    ...
                    recalculate_points($pdo);
                    //set_flash('success', 'Match added and points recalculated.');
                }
    ...
    if ($action === 'delete') {
        ...
        try {
            ...
            recalculate_points($pdo);
            //set_flash('success', 'Match and related scores/awards deleted. Points recalculated.');
        } catch (PDOException $exception) {
...
$matches = $pdo->query(
    'SELECT m.match_id, m.team1_id, m.team2_id, m.match_date, m.winner_team_id,
     ...
     GROUP BY m.match_id, m.team1_id, m.team2_id, m.match_date, m.winner_team_id,
              t1.team_name, t2.team_name, tw.team_name
    ...
    <section class="card card-accent">
        <div class="card-header">
            <div>
                ...
                <p>Winner can remain pending until the match is completed</p>
            </div>
        </div>

        <?php if (count($teams) < 2): ?>
            <div class="alert alert-warning"><span>Create at least two teams before scheduling a match.</span></div>
        <?php endif; ?>
        ...
        <form method="post" class="form-grid">
            ...
            <div class="form-group">
                ...
                <select id="team1_id" name="team1_id" required>
                    ...
                    <?php foreach ($teams as $team): ?>
                        <option value="<?= (int) $team['team_id'] ?>" <?= (int) $editMatch['team1_id'] === (int) $team['team_id'] ? 'selected' : '' ?>><?= h($team['team_name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="team2_id" name="team2_id" required>
                    ...
                    <?php foreach ($teams as $team): ?>
                        <option value="<?= (int) $team['team_id'] ?>" <?= (int) $editMatch['team2_id'] === (int) $team['team_id'] ? 'selected' : '' ?>><?= h($team['team_name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group full">
                ...
                <input id="match_date" name="match_date" type="date" required value="<?= h($editMatch['match_date']) ?>">
            </div>

            <div class="form-group full">
                <label for="winner_team_id">Winner</label>
                <select id="winner_team_id" name="winner_team_id" data-selected-winner="<?= h($editMatch['winner_team_id']) ?>">
                    <option value="">Pending / No winner yet</option>
                </select>
                <span class="help-text">Only Team 1 or Team 2 can be selected as winner.</span>
            </div>

            <div class="form-actions">
                <button class="btn btn-primary" type="submit" <?= count($teams) < 2 ? 'disabled' : '' ?>><?= (int) $editMatch['match_id'] > 0 ? 'Update Match' : 'Add Match' ?></button>
    ...
    <section class="card">
        ...
        <div class="table-toolbar">
            <div class="search-box">
                <input type="search" placeholder="Search team, date or winner..." data-table-search="matchesTable">
            </div>
        </div>
        ...
        <div class="table-wrap">
            <table id="matchesTable">
                ...
                <?php foreach ($matches as $match): ?>
                    <tr>
                        <td>
                            <strong>#<?= (int) $match['match_id'] ?> · <?= h($match['team1_name']) ?></strong><br>
                        ...
                        <td>
                            <span class="badge badge-muted"><?= (int) $match['score_count'] ?> scores</span>
                            <span class="badge badge-muted"><?= (int) $match['award_count'] ?> awards</span>
                        </td>
                        <td>
                            <div class="table-actions">
                                <a class="btn btn-secondary btn-sm" href="matches.php?edit=<?= (int) $match['match_id'] ?>">Edit</a>
                                <form method="post" class="inline-form" data-confirm="Delete this match? Its scores and awards will also be deleted.">
                                    <?= csrf_field() ?>
                ...
                <?php endforeach; ?>
                <?php if (!$matches): ?>
                    <tr><td colspan="5" class="empty-state"><strong>No matches found</strong>Schedule a match using the form.</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
        </div>
